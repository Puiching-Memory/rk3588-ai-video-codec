...
| rockchip-mpp      | 源码子模块  | Media Process Platform，随可移植包携带动态库 |
| libdrm-dev        | 系统包      | DRM/KMS 接口                     |
| patchelf          | 系统包      | 可移植包 RPATH 设置              |
| CMake             | >= 3.21     | 构建系统，支持 CMake Presets     |
| Ninja             | 推荐        | 构建加速                         |
...
**产物**: `rkvc-0.1.4-linux-aarch64-portable.tar.gz` (~2.5 MB)
...
- rockchip-mpp: 从 `third_party/mpp/` 子模块源码构建，随包携带动态库
- ffmpeg-rockchip: 从 `third_party/ffmpeg-rockchip/` 子模块源码构建，链接上述 MPP 构建
- rkvc: 从项目源码构建
...
| 选项           | 说明                           |
| -------------- | ------------------------------ |
| `-i <file>`    | 输入原始视频文件 (NV12)，不是 .h265/.mp4 等压缩文件 |
...
# 已编码 H.265 样本应使用 rkvc_decode 或转码示例，不应传给 rkvc_encode -i
...
| 可视化预览 | `examples/visual_compare.c` | SDL2 原始/重建画面对比与实时指标 |
...
./build/example_visual_compare -i input.h265 -l  # SDL2 可视化预览（可选构建）
```
...
│   ├── test_permissions.c      # 设备权限门控回归测试
│   ├── test_fault_injection.c  # OOM 故障注入测试
│   ├── test_hardware.c         # 硬件编解码集成测试
│   ├── test_scale.c            # RGA 缩放测试
│   └── test_types.c            # 类型定义测试
│
├── scripts/                    # 构建/打包脚本
...
| 症状                 | 可能原因          | 解决方案                                     |
| -------------------- | ----------------- | -------------------------------------------- |
| `RKVC_ERR_PERMISSION` | 设备权限不足     | 检查 `/dev/mpp_service`、`/dev/dma_heap/*`、`/dev/dri/*` 权限 |
| `RKVC_ERR_FORMAT`    | 输入格式不匹配    | 确认编码输入为原始 NV12，压缩码流请走解码或转码路径          |
| `RKVC_ERR_HW`        | 硬件初始化失败    | 检查驱动状态或重启编解码器                   |
...
# 2. 检查设备权限
ls -la /dev/mpp_service /dev/dma_heap /dev/dma_heap/* /dev/dri/* /dev/rga
...
| 矩阵         | 覆盖内容                                       | 命令                                                         |
| ------------ | ---------------------------------------------- | ------------------------------------------------------------ |
| 基线单元测试 | 类型、帧生命周期、API 契约、内部映射、权限门控、OOM 注入 | `cmake --preset tests && cmake --build --preset tests`       |
| 工具脚本     | CLI 参数错误、bench 权限失败传播               | `cmake --preset full-tests && cmake --build --preset full-tests` |
...
| 可移植包     | 二进制完整性、动态库依赖、CLI 功能、开发文件、本机网络回环、负向包检查 | `./scripts/test-portable.sh <package-dir>`                   |

`RKVC_ENABLE_FAULT_INJECTION` 只在测试 preset 中打开，用于确定性模拟项目自有分配失败和 fake `/dev` 权限门控；Release/交付构建默认不包含测试钩子。`test_hardware` 会自动探测 RKMPP 设备节点，不可用时 CTest skip，RK3588 实机交付时必须实际通过。当前 `tests` preset 为 8 个 CTest 目标 / 68 个 CMocka 用例；`full-tests` 为 10 个 CTest 目标。
