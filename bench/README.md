...
| `post-upscale` | **下采样编码 + 上采样后处理**（bench 用 CPU swscale；Session 用 RGA 硬件） |
| `svt-av1+up3x-bilinear` | 单算法路线（`ENC_SCALE_DENOM=3` 时 CSV 名为 `svt-av1+up{N}x-{algo}`） |

## 下采样 + 后处理上采样（评估 NN 占位）

模拟未来「低分辨率编码 → 解码 → NN/传统上采样」管线，当前用 **ffmpeg swscale** 传统插值：
...
| 路径 | 说明 |
|------|------|
| `bench/results/rd_data.csv` | 原始数据 |
| `bench/results/rd_curve_e2e.png` | RD 曲线 |
...
- `UPSCALE_ALGOS` — 上采样算法（bench CPU swscale，Session RGA），逗号分隔（默认 `nearest,bilinear,bicubic`）
